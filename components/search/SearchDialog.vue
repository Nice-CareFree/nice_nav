...
import {get_web_searchKey} from "~/utils/api";
...
const searchHistory = useCookie('search_history',{
  default: () => [],
  watch: 'shallow'
})
...
const getWebSearch = async () => {
  const res:any = await get_web_searchKey()
...
<template>
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="sm:max-w-2xl rounded-xl overflow-hidden">
      <DialogTitle class="sr-only">站内搜索</DialogTitle>
      <div class="p-6">
        <!-- 搜索框 -->
